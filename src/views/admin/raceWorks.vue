    ...
    <el-table :data="tableData">
      ...
      <el-table-column
        ...
        <template slot-scope="scope">
          ...
          <a :href="scope.row.fileUrl" :download="scope.row.fileUrl"
             style="color:#fff;padding:12px 20px;background-color:#409EFF;margin-right:10px;border-radius:4px;"
             @click="downladBefore(scope.row)">下载作品</a>
          <!-- </el-button> -->
...
export default {
  ...
  methods: {
    ...
    getOverList() {
      this.getData("/matchWork/scoreLis", "get").then(res => {
        console.log("已评分：：：", res)
        if (res.data.code == 200) {
    ...
    downladBefore(row) {
      this.getData(`/workerScore/download?workerId=${row.id}`, "get").then(res => {
        console.log("下载埋点:", res)
      })
    },
  },
}
